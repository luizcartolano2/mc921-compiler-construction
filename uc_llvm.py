...
llvm_type = {
    ...
    'void': void_type,
    #'void_*': voidptr_ty,
    'string': charptr_ty #TODO: vai precisar de ptr pra string, né?
}


def create_bytearray(buf):
    """
...
class LLVMFunctionVisitor(BlockVisitor):
    ...
    def _cio(self, fname, format, *target):
        ...
        fmt_bytes = create_bytearray((format + '\00').encode('ascii'))
    ...
    def _build_print(self, val_type, target):
        ...
        """
        if target:
            _value = self._get_location(target)
            if val_type == 'int':
            ...
            if fn_sig:
                for _el in _value:
                    if _el not in list(llvm_type.keys()):
                        fn_sig = False
            if _ctype == "string":
                cte = create_bytearray((_value + "\00").encode('utf-8'))
